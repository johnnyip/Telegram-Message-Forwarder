...
async def bot_send_file(bot: Bot, target, file_path: str, caption: str = "", media_type: Optional[str] = None):
    ...
    with p.open("rb") as fh:
        if media_kind == "photo":
            return await bot.send_photo(chat_id=target, photo=fh, caption=caption, parse_mode="Markdown")
        if media_kind == "video":
            return await bot.send_video(chat_id=target, video=fh, caption=caption, parse_mode="Markdown")
        if media_kind == "animation":
            return await bot.send_animation(chat_id=target, animation=fh, caption=caption, parse_mode="Markdown")
        if media_kind == "audio":
            return await bot.send_audio(chat_id=target, audio=fh, caption=caption, parse_mode="Markdown")
        if media_kind == "voice":
            return await bot.send_voice(chat_id=target, voice=fh, caption=caption, parse_mode="Markdown")
        if media_kind == "video_note":
            return await bot.send_video_note(chat_id=target, video_note=fh)
        return await bot.send_document(chat_id=target, document=fh, caption=caption, parse_mode="Markdown")
